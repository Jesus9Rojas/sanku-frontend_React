...
const ReportesBI = () => {
  ...
  // Referencias para extraer la imagen del Canvas para el PDF
  ...
  useEffect(() => {
    ...
    // SOLUCIÓN 2: Metemos los headers dentro del useEffect para que el linter no pida dependencias
  ...
  const exportarPDF = async () => {
    setLoadingPdf(true);
    sileo.loading({ title: "Preparando PDF..." });
    setTimeout(() => {
      try {
        const doc = new jsPDF('l', 'mm', 'a4');
        doc.setFontSize(20);
        doc.text("Reporte de Inteligencia de Negocios (BI)", 15, 20);

        // Agrega tabla de alertas
        const tableBody = alertas.map(a => [a.alumno || a.nombre, a.promedio_historico || a.promedio, a.carrera || 'General']);
        doc.autoTable({ startY: 30, head: [['Alumno', 'Promedio', 'Carrera']], body: tableBody.length > 0 ? tableBody : [['Sin datos', '', '']] });

        doc.save(`Reporte_BI_${new Date().getTime()}.pdf`);
        sileo.success({ title: "¡PDF Descargado!" });
      } catch { 
        // SOLUCIÓN 1: Quitamos la variable (err)
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={exportarPDF} disabled={loadingPdf} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md flex items-center gap-2 transition-colors disabled:opacity-50"><FileText size={16}/> {loadingPdf ? 'Creando...' : 'Descargar PDF'}</button>
          <button onClick={exportarCSV} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md flex items-center gap-2 transition-colors"><FileDown size={16}/> Descargar CSV</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          ...
          <div className="h-64">{finanzas ? <Bar ref={finanzasRef} data={finanzas} options={{maintainAspectRatio: false, plugins: {legend:{display:false}}}} /> : <div className="h-full flex items-center justify-center text-slate-400"><Activity className="animate-spin"/></div>}</div>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          <h3 className="font-bold text-slate-800 mb-4 border-b border-slate-100 pb-2">Evolución de Matrículas</h3>
          <div className="h-64">{matriculas ? <Line ref={matriculasRef} data={matriculas} options={{maintainAspectRatio: false, plugins: {legend:{display:false}}}} /> : <div className="h-full flex items-center justify-center text-slate-400"><Activity className="animate-spin"/></div>}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-10">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          ...
          <div className="h-60 flex justify-center">{semaforo ? <Doughnut ref={semaforoRef} data={semaforo} options={{maintainAspectRatio: false, cutout: '75%'}} /> : <div className="h-full flex items-center justify-center text-slate-400"><Activity className="animate-spin"/></div>}</div>
        </div>
