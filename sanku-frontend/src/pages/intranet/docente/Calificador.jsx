...
import { FileSignature, CheckCircle2, Save, AlertCircle } from 'lucide-react';
...
const Calificador = () => {
  ...
  const [notas, setNotas] = useState({}); // { [alumnoId]: valorNota }
  ...
  const seleccionarEvaluacion = async (ev) => {
    ...
    try {
      ...
      listaAlumnos.forEach(a => {
        const n = notasPrevias.find(np => parseInt(np.alumnoId) === parseInt(a.alumnoId));
  ...
  const manejarCambioNota = (alumnoId, valorStr) => {
    let valor = valorStr.slice(0, 2); // Máximo 2 dígitos
    if (valor !== '' && parseInt(valor) > 20) valor = '20';
    if (valor !== '' && parseInt(valor) < 0) valor = '0';
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          {cargandoSecciones ? <span className="text-slate-400">Cargando...</span> : 
            secciones.length === 0 ? <span className="text-slate-400">Sin cursos asignados.</span> :
            secciones.map(s => (
              <button 
                key={s.idSeccion} 
        ...
        <div className="flex flex-wrap gap-3 min-h-[44px]">
          {!seccionActual ? <span className="text-slate-400 text-sm">Elige un curso primero...</span> :
           cargandoEvals ? <span className="text-slate-400 text-sm"><i className="fa-solid fa-spinner fa-spin"></i> Buscando evaluaciones...</span> :
           evaluaciones.length === 0 ? <span className="text-rose-500 text-sm font-semibold flex items-center gap-1"><AlertCircle size={16}/> Aún no has creado evaluaciones para este curso.</span> :
           evaluaciones.map(ev => (
             <button 
                key={ev.idEvaluacion} 
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {cargandoNotas ? <tr><td colSpan="2" className="text-center py-10 text-slate-400"><i className="fa-solid fa-spinner fa-spin mr-2"></i> Cargando nómina...</td></tr> :
                  alumnos.length === 0 ? <tr><td colSpan="2" className="text-center py-10 text-slate-400">No hay alumnos matriculados.</td></tr> :
                  alumnos.map((a) => (
